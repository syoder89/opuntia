...
PDU::PDU() :
    ...
    m_date(NULL), m_time(NULL), m_tz(NULL), m_udh_type(NULL), m_udh_data(NULL), m_err(NULL),
...
bool PDU::parseDeliver()
{
    ...
    // Timezone is a +-quarter hour value
    // Swap nibbles, then upper bit is the sign (0=positive, 1=negative)
    // Remaining semi-digits are the number of 15-minute intervals from UTC
    // So 4A becomes A4, upper bit set so negative 24 which is UTC-6
    sprintf(str_buf, "0x%c", m_pdu_ptr[1]);
    int tz=strtol(str_buf, NULL, 0);

    sprintf(str_buf, "%c%d%c", tz & (1L<<3) ? '-':'+',
		    tz & ~(1L<<3), m_pdu_ptr[0]);
    sprintf(m_tz, str_buf);
