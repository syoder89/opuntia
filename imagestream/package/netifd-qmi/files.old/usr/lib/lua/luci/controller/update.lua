...
function index()
	...
	page = entry({"admin", "status", "update"}, template("myapp-mymodule/cbi_tab3"), _("Firmware Update"))
...
function action_get_ver()
	...
	rv["current"] = luci.model.uci.cursor():get("cbi_file", "Version", "ver")
	rv["last"] = luci.model.uci.cursor():get("cbi_file", "Version", "last")
	if rv["last"] == nil then
		rv["last"] = "Not Checked"
	end
...
function action_get_change()
	...
	local err = os.execute("/etc/getlog")
