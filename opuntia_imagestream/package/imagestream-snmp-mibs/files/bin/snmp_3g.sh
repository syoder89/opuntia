...
function create_mib()
{
	...
	if (!empty($tmp)) {
		$carrier = current($tmp);
		$ifnum=`(ip link show dev wwan0 2>/dev/null || ip link show dev usb0 2>/dev/null || echo "1:") | head -n 1 | cut -d ':' -f 1`;
		$tech = next($tmp);
		$mcc = next($tmp);
		$mnc = next($tmp);
...
function ErrorHandler($code, $message, $file, $line) {
	syslog(LOG_ERR, "Error in $file line $line, code $code. Message: $message");
}
