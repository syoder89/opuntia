...
proto_qmi_setup() {
	...
	if [ "${iface:0:3}" = "usb" ] ; then
		proto_qmi_log daemon.info "Detected NDIS instead of QMI"
		. ndis.sh
		proto_ndis_setup $*
		return
	fi
...
#	proto_qmi_log daemon.info "Modem online"
	...
	uqmi -d $CDCDEV --set-network-modes $rat || uqmi -d $CDCDEV --set-network-modes umts,gsm,cdma
		...
		while sleep 20; do
			...
			[ "$STATUS" -gt 0 ] && {
				...
				exit 1
                        ...
                        router=$(ip neigh show dev ${iface} | head -n 1 | cut -d ' ' -f1 )
                        if [ -z $router ] ; then
                                proto_qmi_log daemon.info "Neighbor not found"
                                continue
                        fi
                        if ! arping -q -c1 -w 5 -I ${iface} $router; then
                                let counter=${counter}+1
                                proto_qmi_log daemon.err "Arping gateway $router timed out, Error counter: $counter"
                        else
